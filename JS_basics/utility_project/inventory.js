...
// iii. reportInStock: Logs to the console the item names of all the items that are in stock as a comma separated values
...
let ItemCreator = (() => {
  ...
  function validItemNameLetters(str) {
    chars = str.split('').filter((char) => char.match(/[a-z]/i));
    return chars.length >= 5 ? chars : false;
  }

  function createSKUId(item, category) {
    return validItemNameLetters(item).slice(0, 3).join('') + category.slice(0,2);
  }
...
let ItemManager = {
  ...
  findItem(sku) {
    return this.items.filter((obj) => obj.skuCode === sku);
  },
